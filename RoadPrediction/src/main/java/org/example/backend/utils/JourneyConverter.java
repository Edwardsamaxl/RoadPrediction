...
public class JourneyConverter {
    ...
    public static Journey convertToJourney(Route route) {
    ...
    private static String geocode(String address) throws Exception {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) throw new RuntimeException("Failed to get response");
